...
-- 标签清单（选择制，不是自由填写）：文档从这里挑标签，不是自己打字。
-- documents.tags 还是 text[]（存名字），删掉一个标签不会去清理已经打过标签的文档。
...
-- tags：所有人可读（列表页筛选/文档管理页选择都要读），管理员可写（新建/删除标签）
...
-- 知识库（外部嵌入页面）访问控制：只有"能看/不能看"两档，没有下载概念，
-- 只有站长能设置谁能看到；管理员没有特殊待遇，和普通成员一样要被明确授权。
